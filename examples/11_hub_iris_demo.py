...
def main():
    ...
    print(f"[OK] Model loaded: d_model={adapter.d_model}, n_layers={len(adapter.transformer.blocks)}")
    ...
    if sN.shape[1] > s0.shape[1]:
        print(f"[PASS] Dynamics verification: History grew from {s0.shape[1]} to {sN.shape[1]} tokens.")
        ...
        if not torch.equal(s0[:, -1], sN[:, -1]):
            print(f"[PASS] Token drift: Last token changed from {s0[0, -1].item()} to {sN[0, -1].item()}")
    else:
