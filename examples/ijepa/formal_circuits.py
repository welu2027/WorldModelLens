...
#             self.adapter = IJEPAAdapter.from_checkpoint("ijepa-mini.pth", config)
...
#         raw_perf = 1 - (mse / (baseline_mse + 1e-6))
#         if k is not None:
#             print(
#                 f"K={k:3d} | mse={mse:.6f} | baseline={baseline_mse:.6f} | raw_perf={raw_perf:.6f}"
#             )
#         return raw_perf
#         #print(
#         #    f"K={i+1:3d} | mse={mse:.6f} | baseline={baseline_mse:.6f} | raw_perf={raw_perf:.6f}"
#         #)
#         #performance = 1 - (mse / (baseline_mse + 1e-6))
...
#             with torch.no_grad():
#                 h, _ = self.wm.adapter.encode(self.img_tensor)
#                 pred = self.wm.adapter.predictor(h, self.adapter.last_context_ids,[target_id])
