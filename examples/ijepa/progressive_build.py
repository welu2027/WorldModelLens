...
def animate_ijepa_progressive(seed: int = 42):
    ...
    def update(frame):
        ...
        # 5. Run Prediction with subset of context
        with torch.no_grad():
            # In real I-JEPA, we'd just slice the context latents
            ...
            # Compute Cosine Similarity as "Quality"
        ...
        # Draw background context (all 40 context patches)
        nx.draw_networkx_nodes(
            nx.Graph(), pos={f"patch_{pid}": get_coord(f"patch_{pid}") for pid in context_ids}, 
            nodelist=[f"patch_{pid}" for pid in context_ids], 
            node_color="#eeeeee", node_size=300, alpha=0.2, ax=ax1
        )
